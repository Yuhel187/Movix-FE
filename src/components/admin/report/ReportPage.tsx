...
import {
  ...
  MessageSquare, Loader2, Heart, FileText, DollarSign, CreditCard, PieChart as PieIcon
} from "lucide-react";
...
const chartConfig = {
  ...
  refund: { label: "Hoàn tiền", color: "var(--chart-5)" },
...
type ReportTabId = "overview" | "content" | "users" | "financial";
const tabs: { id: ReportTabId; label: string; icon: React.ElementType }[] = [
  { id: "overview", label: "Tổng quan", icon: TrendingUp },
  { id: "content", label: "Nội dung", icon: Film },
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [reportRes, topMoviesRes, statsRes, financialRes] = await Promise.allSettled([
          apiClient.get('/dashboard/report-all', { params }),
          apiClient.get('/dashboard/top-movies'),
          apiClient.get('/dashboard/stats'),
          adminReportService.getFinancialReport(
            dateRange?.from ? format(dateRange.from, 'yyyy-MM-dd') : format(addDays(new Date(), -30), 'yyyy-MM-dd'),
  ...
  const handleExportReport = async () => {
    ...
    try {
      ...
      if (activeTab === 'overview') {
        ...
        doc.setFontSize(12);
        doc.setTextColor(0);
        doc.text("1. Tóm tắt tài chính", 14, currentY);
        currentY += 6;

        autoTable(doc, {
          startY: currentY,
          head: [['Doanh thu', 'Hoàn tiền', 'Thực thu (Net)', 'Số GD']],
          body: [[
            (financialData.summary.totalRevenue || 0).toLocaleString('vi-VN') + " VND",
            (financialData.summary.totalRefund || 0).toLocaleString('vi-VN') + " VND",
            (financialData.summary.netRevenue || 0).toLocaleString('vi-VN') + " VND",
            (financialData.summary.totalTransactions || 0).toLocaleString('vi-VN')
          ]],
          theme: 'grid',
          headStyles: { fillColor: [50, 50, 50], textColor: 255, ...tableStyles, fontStyle: 'bold' as const },
          bodyStyles: { textColor: 0, fontSize: 11, halign: 'center', ...tableStyles },
        ...
        doc.setFontSize(12);
        doc.text("2. Phân bổ doanh thu theo Gói cước", 14, currentY);
        currentY += 6;

        const planRows = (financialData.planBreakdown.transactionByPlan || []).map((p: any) => [
          p.plan_name || p.plan_id || "Không xác định",
          p._count.id.toLocaleString('vi-VN'),
          (p._sum.amount || 0).toLocaleString('vi-VN') + " VND"
        ]);

        autoTable(doc, {
          startY: currentY,
          head: [['Mã Gói Cước', 'Số Giao Dịch', 'Tổng Doanh Thu']],
          body: planRows,
          theme: 'striped',
          headStyles: { fillColor: [229, 9, 20], ...tableStyles, fontStyle: 'bold' as const },
          bodyStyles: { ...tableStyles },
        ...
        {activeTab === 'financial' && financialData && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <Card className="bg-[#262626] border-slate-800 text-white hover:border-slate-600 transition-colors">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium text-gray-300">Tổng Doanh thu</CardTitle>
                  <DollarSign className="h-4 w-4 text-green-500" />
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold">{(financialData.summary.totalRevenue || 0).toLocaleString()} <span className="text-sm font-normal text-gray-400">VND</span></div>
                  <p className="text-xs text-gray-400 mt-1">Trước khi trừ hoàn tiền</p>
                </CardContent>
              </Card>

              <Card className="bg-[#262626] border-slate-800 text-white hover:border-slate-600 transition-colors">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium text-gray-300">Tổng Hoàn tiền</CardTitle>
                  <TrendingUp className="h-4 w-4 text-red-500 rotate-180" />
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold">{(financialData.summary.totalRefund || 0).toLocaleString()} <span className="text-sm font-normal text-gray-400">VND</span></div>
                  <p className="text-xs text-gray-400 mt-1">Các giao dịch đã Refund</p>
                </CardContent>
              </Card>

              <Card className="bg-primary/10 border-primary/30 text-white hover:border-primary/50 transition-colors">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium text-white">Doanh thu thực (Net)</CardTitle>
                  <TrendingUp className="h-4 w-4 text-primary" />
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold">{(financialData.summary.netRevenue || 0).toLocaleString()} <span className="text-sm font-normal text-gray-400">VND</span></div>
                  <p className="text-xs text-gray-300 mt-1">Lợi nhuận thực tế</p>
                </CardContent>
              </Card>

              <Card className="bg-[#262626] border-slate-800 text-white hover:border-slate-600 transition-colors">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium text-gray-300">Giao dịch</CardTitle>
                  <CreditCard className="h-4 w-4 text-blue-500" />
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold">{financialData.summary.totalTransactions?.toLocaleString()}</div>
                  <p className="text-xs text-gray-400 mt-1">Số lượng hóa đơn</p>
                </CardContent>
              </Card>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Revenue vs Refund Chart */}
              <Card className="bg-[#262626] border-slate-800 text-white">
                <CardHeader>
                  <CardTitle className="text-lg">So sánh Thu - Chi</CardTitle>
                  <CardDescription>Tỷ lệ giữa doanh thu và tiền hoàn trả.</CardDescription>
                </CardHeader>
                <CardContent className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={[
                          { name: 'Thực thu', value: financialData.summary.netRevenue },
                          { name: 'Hoàn tiền', value: financialData.summary.totalRefund },
                        ]}
                        cx="50%"
                        cy="50%"
                        innerRadius={60}
              ...
              <Card className="bg-[#262626] border-slate-800 text-white">
                <CardHeader>
                  <CardTitle className="text-lg">Doanh thu theo Gói cước</CardTitle>
                  <CardDescription>Chi tiết nguồn thu từ các gói Membership.</CardDescription>
                </CardHeader>
                <CardContent className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={financialData.planBreakdown.transactionByPlan} margin={{ bottom: 20 }}>
                      <CartesianGrid strokeDasharray="3 3" strokeOpacity={0.1} vertical={false} />
                      <XAxis
                        dataKey="plan_name"
                        stroke="#888"
                      ...
                      <RechartsTooltip
                        ...
                        formatter={(value: number) => value.toLocaleString() + " VND"}
                      />
                      <Bar dataKey="_sum.amount" name="Doanh thu" fill="var(--chart-3)" radius={[4, 4, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
