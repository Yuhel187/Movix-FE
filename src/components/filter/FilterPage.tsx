...
export default function FilterPage({
  ...
  const fetchMovies = useCallback(
    async (currentFilters: FilterState, page: number) => {
      setIsLoading(true);
      setError(null);
      ...
      if (currentFilters.type && currentFilters.type !== "Tất cả") {
        const typeValue =
          currentFilters.type === "Phim lẻ"
            ? "phim-le"
            : currentFilters.type === "Phim bộ"
            ? "phim-bo"
            : currentFilters.type;
      ...
      if (currentFilters.country && currentFilters.country !== "Tất cả")
        params.append("country", currentFilters.country);
      if (currentFilters.year && currentFilters.year !== "Tất cả")
  ...
  const handleImageSearch = async () => {
    ...
    try {
      const res = await apiClient.post("/ai/search-image", formData, {
  ...
  useEffect(() => {
    ...
    // Reset image search state when URL changes (user navigates normally)
    clearImage();
    setShowImageSearch(false);
  ...
  const updateURL = (newFilters: FilterState) => {
    ...
    if (newFilters.type && newFilters.type !== "Tất cả") {
      const typeValue =
        newFilters.type === "Phim lẻ"
          ? "phim-le"
          : newFilters.type === "Phim bộ"
          ? "phim-bo"
          : newFilters.type;
    ...
    if (newFilters.country && newFilters.country !== "Tất cả")
      params.set("country", newFilters.country);
    if (newFilters.year && newFilters.year !== "Tất cả")
            ...
            <div className="flex flex-wrap gap-4 items-center justify-between">
              ...
              <div className="flex gap-3">
                <Button
                  ...
                  disabled={isAiSearching || showImageSearch}
                >
                ...
                >
                  <FaImage /> {showImageSearch ? "Đóng" : "Tìm bằng Ảnh"}
                </Button>
              ...
              {showAISearch && (
                ...
                >
                  ...
                  <p className="text-xs text-purple-400/60 mt-2 italic">
                    * AI sẽ phân tích ngữ nghĩa câu nói của bạn để tìm phim phù
                    hợp nhất trong kho dữ liệu.
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
                  ...
                  <div className="flex flex-col items-center">
                    {!selectedImage ? (
                      <div
                        ...
                        className="w-full max-w-xl border-2 border-dashed border-green-500/50 rounded-lg p-10 flex flex-col items-center justify-center cursor-pointer hover:bg-green-900/10 hover:border-green-400 transition-all"
                      >
                        <FaImage className="text-5xl text-green-400/50 mb-3" />
                      ...
                      <div className="flex flex-col items-center gap-4 w-full">
                        <div className="relative group">
                          ...
                          <img
                            src={imagePreview || ""}
                            alt="Preview"
                            className="max-h-[300px] w-auto rounded-lg shadow-2xl border border-green-500/30 object-contain"
                          />
                          <button
                            onClick={clearImage}
                            className="absolute -top-3 -right-3 bg-red-600 rounded-full p-2 text-white shadow-lg hover:bg-red-700 transition-transform hover:scale-110"
                            title="Xóa ảnh"
                          >
                            <FaTimes size={14} />
                          </button>
